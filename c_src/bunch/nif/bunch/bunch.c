...
/**
 * Builds `{:error, reason_atom}` where `reason_atom` is created out of `reason`.
 */
...
ERL_NIF_TERM bunch_make_error_errno(ErlNifEnv* env, const char* call) {
  ERL_NIF_TERM tuple = enif_make_tuple2(
    env,
    enif_make_atom(env, bunch_errno_to_string(errno)),
...
/**
 * Builds error description for invalid argument error and raises using `bunch_raise_error`.
 */
ERL_NIF_TERM bunch_raise_error_args(ErlNifEnv* env, const char* field, const char *reason) {
  ...
  sprintf(description, "Invalid argument %s, reason: %s", field, description);
...
ERL_NIF_TERM bunch_raise_error(ErlNifEnv* env, const char* description) {
  return enif_raise_exception(env, enif_make_string(env, description, ERL_NIF_LATIN1));
}
