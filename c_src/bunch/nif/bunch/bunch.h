...
#define BUNCH_PARSE_UINT_ARG(position, var_name) \
  BUNCH_PARSE_ARG(position, var_name, unsigned int var_name, enif_get_uint, &var_name)
...
#define BUNCH_PARSE_STRING_ARG(position, var_name, max_size) \
  BUNCH_PARSE_ARG(position, var_name, char var_name[max_size], enif_get_string, (char *) var_name, max_size, ERL_NIF_LATIN1)

#define BUNCH_PARSE_BINARY_ARG(position, var_name) \
  BUNCH_PARSE_ARG(position, var_name, ErlNifBinary var_name, enif_inspect_binary, &var_name)
